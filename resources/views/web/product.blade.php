	...
	<div class="container">
		<div class="row">
			...
			<div class="col-lg-6">
				...
				@endif
				<p class="text-left text-dark">Categorías:
					@foreach($product->subcategories()->with('category')->get()->pluck('category')->unique('id')->values() as $category)
					<a href="{{ route('shop', ['category' => $category->slug]) }}">{{ $category->name }}</a>@if(!$loop->last){{ ", " }}@endif
					@endforeach
				</p>
				<p class="text-left text-dark">Subcategorías:
					@foreach($product->subcategories as $subcategory)
					<a href="{{ route('shop', ['category' => $subcategory->category->slug, 'subcategory' => $subcategory->slug]) }}">{{ $subcategory->name }}</a>@if(!$loop->last){{ ", " }}@endif
					@endforeach
				</p>
				<div class="row">
